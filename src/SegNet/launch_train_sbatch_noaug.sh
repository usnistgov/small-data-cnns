...
#SBATCH -o segnet_noaug_%N.%j.out
#SBATCH --mail-user=<your_email_here>@nist.gov
#SBATCH --mail-type=FAIL
#SBATCH --time=48:0:0

timestamp="$(date +%Y%m%dT%H%M%S)"
experiment_name="segnet-noaug-${timestamp}"
...
python train_segnet.py --test_every_n_steps=${test_every_n_steps} --batch_size=${batch_size} --train_database="$working_dir/$train_lmdb_file" --test_database="$working_dir/$test_lmdb_file" --output_dir="$results_dir" --use_augmentation=0 --balance_classes=1 --number_classes=4 | tee "$results_dir/log.txt"
