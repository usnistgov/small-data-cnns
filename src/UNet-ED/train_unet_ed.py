...
parser.add_argument('--train_database', dest='train_database_filepath', type=str, help='lmdb database to use for training (Required)', required=True)
...
import unet_ed_model
import imagereader_ed
...
def train_model():
    ...
    train_reader = imagereader_ed.ImageReader(train_lmdb_filepath, batch_size=batch_size, use_augmentation=use_augmentation, shuffle=True, num_workers=READER_COUNT)
    print('Train Reader has {} batches'.format(train_reader.get_epoch_size()))

    print('Setting up training image reader')
    test_reader = imagereader_ed.ImageReader(test_lmdb_filepath, batch_size=batch_size, use_augmentation=0, shuffle=False, num_workers=READER_COUNT)
    print('Train Reader has {} batches'.format(test_reader.get_epoch_size()))
    ...
    try: # if any errors happen we want to catch them and shut down the multiprocess readers
        ...
        with tf.Graph().as_default(), tf.device('/' + gradient_update_location):
            train_init_op, test_init_op, train_op, loss_op, is_training_placeholder = unet_ed_model.build_towered_model(train_reader, test_reader, GPU_IDS, learning_rate)
            ...
            while True: # loop until early stopping
                ...
                for step in range(adj_batch_count):
                    _, loss_val = sess.run([train_op, loss_op], feed_dict={is_training_placeholder: True})
                ...
                for step in range(adj_batch_count):
                    [loss_val] = sess.run([loss_op], feed_dict={is_training_placeholder: False})
