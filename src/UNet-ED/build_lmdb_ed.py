...
def write_img_to_db(txn, img, key_str):
...
def generate_database(img_list, database_name, image_filepath):
    output_image_lmdb_file = os.path.join(output_filepath, database_name)

    if os.path.exists(output_image_lmdb_file):
        print('Deleting existing database: {}'.format(output_image_lmdb_file))
        shutil.rmtree(output_image_lmdb_file)

    print('Building new lmdb')
    ...
    for i in range(len(img_list)):
        ...
        txn_nb += 1
        write_img_to_db(image_txn, img, key_str)
...
if __name__ == "__main__":
    ...
    print("Found {} tif files".format(len(img_files)))
    ...
    database_name = 'ed-train-{}.lmdb'.format(dataset_name)
    generate_database(train_img_files, database_name, image_folder)

    print('building test database')
    database_name = 'ed-test-{}.lmdb'.format(dataset_name)
    generate_database(test_img_files, database_name, image_folder)
